function TMM3(N,P,wavelength,Ef,T,gamma,indices,conductivity,layers,theta)
%% INSTRUCTIONS.
...
% assumed as combination of the previous scrips [1,2]: considering the 
% conductivity on the two interfaces and a dielectric (n+ik) bulk of 
% N-graphene monolayers. In the last two cases, the conductivity of the 
% graphene is evaluated following the Kubo model with simulateneous 
% contribution of the interbands and intrabands effects for a given 
% chemical potential (Fermi energy), temperatire (T) and a certain 
% relaxation frequency (gamma).
%
% Figure 1 shows the reflectance for TE polarization, figure 2 depicts the
% reflectance for TM polarization and figure 3 represents the comparative 
% between TE/TM response. The method used in this Matlab function is the
% Matrix Transfer Method [3].
%
...
%
% example:TMM3(4,[0.45],632.8,[1.61,1,2.56,1.333],[0,1,0,0],[NaN,0.34,1000,NaN],linspace(0*pi/180,85*pi/180,1000))
...
for q2=1:length(P)
% Graphene permitivity (e=e'+ie'',e'=1 and e''=sigma/e0*w*thickness)    
...
figure(1)
hold on
plot(theta*180/pi,abs(A1rTE(q2,:)).^2,'-')
set(gca,'FontSize',20)
xlim([0 85])
title('Reflectance TE','FontSize',30,'interpreter','tex')
xlabel('Angle of incidence (deg)','FontSize',30,'interpreter','tex')
ylabel('Reflectance ratio (arbitrary units)','FontSize',30,'interpreter','tex')
% Plotting TM reflectance map
figure(2)
hold on
plot(theta*180/pi,abs(A1rTM(q2,:)).^2,'-')
set(gca,'FontSize',20)
xlim([0 85])
title('Reflectance TM','FontSize',30,'interpreter','tex')
xlabel('Angle of incidence (deg)','FontSize',30,'interpreter','tex')
ylabel('Reflectance ratio (arbitrary units)','FontSize',30,'interpreter','tex')
% Plotting TE vs TM reflectance map
figure(3)
hold on
plot(theta*180/pi,abs(A1rTE(q2,:)).^2,'-b')
plot(theta*180/pi,abs(A1rTM(q2,:)).^2,'-r')
set(gca,'FontSize',20)
xlim([0 85])
...
toc
%% References
% [2] Zhan, T., Shi, X., Dai, Y., Liu, X., & Zi, J. (2013). Transfer matrix
...
% Matter, 25(21), 215301.
%
% [3] Born, M., & Wolf, E. (2013). Principles of optics: electromagnetic
